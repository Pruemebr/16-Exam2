...
def problem4b(list_of_tuples_of_strings):
    ...
    #                    AND HAS AN ELEGANT SOLUTION.  DO NOT GET STUCK ON IT!
    # -------------------------------------------------------------------------
    for k in range(len(list_of_tuples_of_strings)): #goes through whole list of tuples
        for j in range(len(list_of_tuples_of_strings[k])): #goes through each string in each tuple
            tuple = list_of_tuples_of_strings[k]
            if is_prime(len(tuple[j])) is True:
                return True
    return False
